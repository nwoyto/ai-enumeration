...
class SpaceNetBuildingDataset(Dataset):
    """
    PyTorch Dataset for SpaceNet data. Reads from local directories
    within a SageMaker training container.
    """
    def __init__(self, image_dir, mask_dir, augment_mode='train', num_input_channels=8, dataset_mean=None, dataset_std=None):
        self.image_dir = Path(image_dir)
        self.mask_dir = Path(mask_dir)
        self.num_input_channels = num_input_channels

        if dataset_mean is None or dataset_std is None:
            logger.warning("Normalization stats not provided. Using a default of 0.5 for mean/std.")
            self.dataset_mean = [0.5] * num_input_channels
            self.dataset_std = [0.5] * num_input_channels
        else:
        ...
        all_image_files = sorted(list(self.image_dir.glob('*.tif')))
        all_mask_files = sorted(list(self.mask_dir.glob('*.tif')))

        image_id_to_path = {self._extract_id(p.name): p for p in all_image_files}
        mask_id_to_path = {self._extract_id(p.name): p for p in all_mask_files}

        common_ids = sorted(list(image_id_to_path.keys() & mask_id_to_path.keys()))
        
        self.data_pairs = [{'image_path': image_id_to_path[id], 'mask_path': mask_id_to_path[id]} for id in common_ids]
        
        if not self.data_pairs:
            raise ValueError(f"No matching image-mask pairs found in {image_dir} and {mask_dir}")
            
        logger.info(f"Initialized dataset in '{augment_mode}' mode with {len(self.data_pairs)} pairs.")

    def _extract_id(self, filename):
        """Extracts the base ID from a filename, removing suffixes like '_mask'."""
        return filename.replace('_mask', '').split('.')[0]

    def __len__(self):
        return len(self.data_pairs)

    def _get_transforms(self, augment_mode):
        """Defines the Albumentations pipeline."""
        base_transforms = [
            A.Resize(224, 224),
            A.Normalize(mean=self.dataset_mean, std=self.dataset_std, max_pixel_value=1.0),
            ToTensorV2(),
        ]
        if augment_mode == 'train':
            return A.Compose([
    ...
    def __getitem__(self, idx):
        ...
        with rasterio.open(data_info['image_path']) as src_img:
            bands_to_read = list(range(1, min(src_img.count, self.num_input_channels) + 1))
            image = src_img.read(bands_to_read).astype(np.float32) / 65535.0

        with rasterio.open(data_info['mask_path']) as src_mask:
            # Keep mask as float32, as required by the loss function
            mask = src_mask.read(1).astype(np.float32)
        ...
        # Ensure the mask is float32 and add the channel dimension
        mask_tensor = augmented['mask'].to(torch.float32).unsqueeze(0)

        return image_tensor, mask_tensor

def calculate_stats(image_dir, num_channels, sample_frac=0.25):
    """Calculates normalization statistics from a local directory of images."""
    logger.info(f"Calculating normalization stats from local directory: {image_dir}")
    image_files = list(Path(image_dir).glob("*.tif"))
    if not image_files:
        raise FileNotFoundError(f"No .tif files found in the directory: {image_dir}")

    if sample_frac < 1.0:
        num_samples = int(len(image_files) * sample_frac)
        image_files = random.sample(image_files, max(1, num_samples))
    
    logger.info(f"Using {len(image_files)} images for calculation (sample fraction: {sample_frac}).")

    channel_sums = np.zeros(num_channels, dtype=np.float64)
    channel_sq_sums = np.zeros(num_channels, dtype=np.float64)
    total_pixels = 0

    for path in tqdm(image_files, desc="Calculating Stats"):
        with rasterio.open(path) as src:
            bands_to_read = list(range(1, min(src.count, num_channels) + 1))
            img_data = src.read(bands_to_read).astype(np.float32) / 65535.0
            
            total_pixels += img_data.shape[1] * img_data.shape[2]
            channel_sums[:len(bands_to_read)] += img_data.sum(axis=(1, 2))
            channel_sq_sums[:len(bands_to_read)] += (img_data**2).sum(axis=(1, 2))

    mean = channel_sums / total_pixels
    std = np.sqrt(channel_sq_sums / total_pixels - mean**2)
