...
###########################################################################
    ...
    # code/train.py - FINAL, CLEANED VERSION (Guaranteed to resolve UnboundLocalError)
    # code/train.py - FINAL, CLEANED VERSION (Guaranteed to resolve UnboundLocalError)
    # code/train.py - FINAL, CLEANED VERSION (Guaranteed to resolve UnboundLocalError)
    # code/train.py - FINAL, CLEANED VERSION (Guaranteed to resolve UnboundLocalError)
    # code/train.py - FINAL, CLEANED VERSION (Guaranteed to resolve UnboundLocalError)
...
from pathlib import Path # Import Path for robust directory listing

# --- SCRIPT VERSION IDENTIFIER ---
# This will help confirm which version of the script is running in CloudWatch logs
SCRIPT_VERSION = "2025-07-23_v1.0.7_final_train_fix_confirmed_again" 
...
# Setup logging
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO) # Changed to INFO for less verbosity in logs unless needed
logger.addHandler(logging.StreamHandler(sys.stdout))
...
def plot_curves(train_losses, val_losses, val_ious, val_dices, epoch, best_val_iou, best_iou_epoch_num, output_img_dir):
    ...
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Loss', fontsize=12)
    plt.title(f'Training & Validation Loss Over Epochs (Epoch {epoch + 1})', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.tight_layout()
    plt.savefig(os.path.join(output_img_dir, f'loss_curve_epoch{epoch}.png'))
    ...
    if best_val_iou > -1.0:
        plt.scatter(best_iou_epoch_num, best_val_iou, color='darkgreen', marker='*', s=200, zorder=5, label=f'Best IoU: {best_iou_epoch_num}')
    ...
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Metric Value', fontsize=12)
    plt.title(f'Validation Metrics (IoU & Dice) Over Epochs (Epoch {epoch + 1})', fontsize=14)
    plt.legend(fontsize=10)
    plt.grid(True, linestyle=':', alpha=0.7)
    plt.ylim(0, 1.05)
    plt.tight_layout()
...
def generate_summary_report(args, best_val_iou, best_iou_epoch_num, train_losses, val_losses, val_dices, output_img_dir, metrics_table):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"**Model:** HybridGeoNet\n")
        f.write(f"**Dataset:** SpaceNet Las Vegas Building Detection\n\n")
        ...
        f.write(f"### 👁️‍🗨️ Example Building Detections (Last Epoch: {args.epochs})\n")
        for i in range(min(args.num_vis_samples, 3)):
            f.write(f"#### Sample {i+1}\n")
            f.write(f"![Sample {i+1} Prediction Overlay](images/val_pred_overlay_epoch{args.epochs-1}_sample{i}.png)\n\n")
...
def train(args):
    logger.info(f"Starting training on {args.device}...")
    logger.info(f"Script version: {SCRIPT_VERSION}") # Log the version
    logger.info(f"Hyperparameters: {args}")

    device = torch.device(args.device)

    # Data paths are passed directly as args from SageMaker (these are local container paths)
    train_image_dir = args.train_images_dir
    train_mask_dir = args.train_masks_dir
    val_image_dir = args.val_images_dir
    val_mask_dir = args.val_masks_dir
    ...
    # Debug: List files in image/mask directories (uses the globally imported 'os' and 'pathlib.Path')
    # This will be printed in CloudWatch logs, crucial for debugging dataset issues
    logger.info(f"Contents of train_images_dir ({train_image_dir}): {os.listdir(Path(train_image_dir)) if Path(train_image_dir).exists() else 'NOT FOUND'}")
    logger.info(f"Contents of train_masks_dir ({train_mask_dir}): {os.listdir(Path(train_mask_dir)) if Path(train_mask_dir).exists() else 'NOT FOUND'}")
    logger.info(f"Contents of val_images_dir ({val_image_dir}): {os.listdir(Path(val_image_dir)) if Path(val_image_dir).exists() else 'NOT FOUND'}")
    logger.info(f"Contents of val_masks_dir ({val_mask_dir}): {os.listdir(Path(val_mask_dir)) if Path(val_mask_dir).exists() else 'NOT FOUND'}")


    # Correct instantiation of SpaceNetBuildingDataset - only pass local root directories
    train_dataset = SpaceNetBuildingDataset(
    ...
    # Crucial check: Ensure datasets are not empty before creating DataLoaders
    if len(train_dataset) == 0:
        raise ValueError("Train dataset has 0 samples. Please check data paths and preprocessing.")
    if len(val_dataset) == 0:
        raise ValueError("Validation dataset has 0 samples. Please check data paths and preprocessing.")
    ...
    for epoch in range(args.epochs):
        ...
        generate_summary_report(args, best_val_iou, best_iou_epoch_num, train_losses, val_losses, val_dices, output_img_dir, metrics_table)
