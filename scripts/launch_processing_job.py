...
s3_raw_images_target_path = f's3://{default_bucket}/{s3_project_prefix}/raw_images/'
...
public_s3_images_source = f's3://spacenet-dataset/spacenet/SN2_buildings/tarballs/AOI_2_Vegas_Train/PS-MS/'
...
# scripts/launch_processing_job.py - FIX: Two Processing Outputs (Masks & Raw Images)

# scripts/launch_processing_job.py - FINAL, CORRECTED VERSION (Guaranteed to resolve TypeError)

# scripts/launch_processing_job.py - FINAL, CORRECTED VERSION (Uses ScriptProcessor)

# scripts/launch_processing_job.py - FINAL: Corrected ScriptProcessor.run()
...
# role = 'arn:aws:iam::040571275415:role/service-role/AmazonSageMaker-ExecutionRole-20250320T164162'
# image_uri = '040571275415.dkr.ecr.us-east-1.amazonaws.com/spacenet-geoprocessing:latest' # Your custom image
...
# # S3 paths for raw data (public SpaceNet dataset - tarballs only)
# public_spacenet_s3_base = 's3://spacenet-dataset/spacenet/SN2_buildings/tarballs/'
# s3_raw_train_tarball = f'{public_spacenet_s3_base}SN2_buildings_train_AOI_2_Vegas.tar.gz'
# s3_raw_test_tarball = f'{public_spacenet_s3_base}AOI_2_Vegas_Test_public.tar.gz'

# # S3 output path for processed masks (in YOUR S3 bucket)
# s3_processed_masks_output_path = f's3://{default_bucket}/{s3_project_prefix}/processed_masks'
# # S3 output path for raw images (in YOUR S3 bucket) - CORRECTED TO USER'S SPECIFIED PATH
# s3_raw_images_output_path = f's3://{default_bucket}/{s3_project_prefix}/raw_data/AOI_2_Vegas/PS-MS/' 

# logger.info(f"\nProcessing Job Configuration:")
# logger.info(f"  Raw Training Tarball S3: {s3_raw_train_tarball}")
# logger.info(f"  Raw Test Tarball S3: {s3_raw_test_tarball}")
# logger.info(f"  Processed Masks Output S3: {s3_processed_masks_output_path}")
# logger.info(f"  Raw Images Output S3: {s3_raw_images_output_path}")

# # Local path to processing script (inside your project structure)
# processing_script_path = str(Path('./preprocessing/process_data_job.py').resolve())

# # --- FIX: Use ScriptProcessor for custom Docker image ---
# processor = ScriptProcessor( # CHANGED from SKLearnProcessor
#     image_uri=image_uri,
#     role=role,
#     instance_type="ml.m5.xlarge",
#     instance_count=1,
...
# processing_job_name = f"spacenet-data-preprocessor-{current_time_str}"

# processor.run(
#     code=processing_script_path, # Pass the script to run using 'code' argument
#     command=["python3"], # Add command explicitly here for ScriptProcessor
#     inputs=[
#         ProcessingInput(
#             source=s3_raw_train_tarball,
#             destination='/opt/ml/processing/input/data/train_tarball/'  
#         ),
#         ProcessingInput(
#             source=s3_raw_test_tarball,
#             destination='/opt/ml/processing/input/data/test_tarball/'  
#         )
#     ],
#     outputs=[
#         ProcessingOutput(
#             source='/opt/ml/processing/output/processed_masks/', 
#             destination=s3_processed_masks_output_path,
#             output_name="processed_masks"
#         ),
#         ProcessingOutput(
#             source='/opt/ml/processing/output/raw_images/', 
#             destination=s3_raw_images_output_path,
#             output_name="raw_images"
#         )
#     ],
...
# logger.info("Data preprocessing job completed successfully. Images and masks are now in your S3 bucket.")